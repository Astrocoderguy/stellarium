...
#ifdef SG_NAV_BAR
	// Construct the nav buttons bar
	navBar = new LeftStelBar(this);
#endif
...
#ifdef SG_NAV_BAR
	const qreal newNavX = ww-navBar->boundingRect().width()-10;
	const qreal newNavY = hh-navBar->boundingRect().height()-30;
	navBar->setPos(newNavX, newNavY);
//	navBar->setZValue(-0.1);
#endif
...
#ifdef SG_NAV_BAR
		navBar->setColor(QColor::fromRgbF(0.9, 0.33, 0.33, 0.9));
		navBar->setRedMode(true);
#endif
...
#ifdef SG_NAV_BAR
		navBar->setColor(QColor::fromRgbF(0.9, 0.91, 0.95, 0.9));
		navBar->setRedMode(false);
#endif
