...
{
	if (action == NULL || action!=NULL && !action->isCheckable())
		setChecked(toggleChecked(checked));
#if SG_NAV_BAR
	emit(pressed(false));
...
#ifdef SG_NAV_BAR_DEL
RightStelBar::RightStelBar(QGraphicsItem* parent) : QGraphicsItem(parent)
{
...
RightStelBar::~RightStelBar()
{
}

void RightStelBar::addButton(StelButton* button)
{
	double posY = 0;
...
void RightStelBar::paint(QPainter*, const QStyleOptionGraphicsItem*, QWidget*)
{
}

QRectF RightStelBar::boundingRect() const
{
	return childrenBoundingRect();
}

QRectF RightStelBar::boundingRectNoHelpLabel() const
{
...
void RightStelBar::buttonHoverChanged(bool b)
{
